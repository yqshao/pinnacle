...
@click.option(
    "--log", metavar="", default=None, help="gromacs/lammps log (for energies)"
)
@click.option("--top", metavar="", default=None, help="gromacs topology")
@click.option("--units", metavar="", default=None, help="lammps units")
@click.option(
    "--emap", metavar="", default=None, help='remap lammps elements, e.g. "1:1,2:8"'
)
...
@click.option(
    "--seed", metavar="", default="0", type=int, help="seed for random number (int)"
)
def convertds(filename, format, output, oformat, shuffle, seed, log, units, emap, top):
    ...
    if ":" in output:
        split = True
        writers = [
            get_writer(s.split(":")[0], format=oformat) for s in output.split(",")
        ]
    ...
    for fname in filename:
        ...
        if split:
            ...
            writerList = sum(
                [
                    [writer] * math.ceil(count * weight / sum(weights))
...
@click.command(
    name="filter", context_settings=CONTEXT_SETTINGS, short_help="filter datasets"
)
@click.argument("filename", nargs=-1)
@click.option(
    "--log", metavar="", default=None, help="gromacs/lammps log (for energies)"
...
@click.option(
    "-amin", "--abs-min", metavar="", default="", help="minimum absolute value"
)
@click.option(
    "-amax", "--abs-max", metavar="", default="", help="maximum absolute value"
)
def filterds(filename, log, units, emap, format, output, oformat, algo, top, **kwargs):
    ...
    datasets = [
        read(fname, log=log, format=format, emap=emap, units=units, top=top)
        for fname in filename
    ]
    if algo == "qbc":
