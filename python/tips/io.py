...
class ase_writer:
    ...
    def add(self, data):
        ...
        atoms.calc = SinglePointCalculator(
            atoms=atoms, energy=data["e_data"], forces=data["f_data"]
        )
        atoms.pbc = True
...
"""
        ...
        self.f.writelines(
            ...
            for i, (elem, coord) in enumerate(zip(data["elems"], data["coord"]))
        )
...
class pinn_writer:
    ...
    def _serialize(self, tensors):
        ...
        return {
            k: tf.io.serialize_tensor(tf.constant(v, dtype=self.spec[k]["dtype"]))
    ...
    def add(self, data):
        ...
        example = tf.train.Example(
            features=tf.train.Features(
                feature={
                    key: self._bytes_feature(val.numpy())
